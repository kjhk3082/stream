...
def render_model_index():
    ...
    with tab2:
        ...
        **수식**:
        ...
        try:
            st.latex(r'''
            적합도점수 = w_1 \cdot S_1 + w_2 \cdot S_2 + w_3 \cdot S_3
            ''')
        except:
            st.markdown("""
            <div style="background-color: #f0f2f6; padding: 15px; border-radius: 10px; text-align: center; font-size: 18px;">
            <b>적합도 점수 = w₁ × S₁ + w₂ × S₂ + w₃ × S₃</b>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.markdown("""
        **변수 설명**:
        - **S₁, S₂, S₃**: 각 항목의 정규화 점수 (0-100점)
          - S₁: 수출액 점수, S₂: 성장률 점수, S₃: 안전도 점수
        - **w₁, w₂, w₃**: 각 항목의 가중치 (합계 100%)
        """)
    ...
    with tab3:
        ...
        **🎯 정규화의 효과**:
        ...
        # 수출 중심 전략 적용 (60%, 25%, 15%)
        st.markdown("**수출 중심 전략 (60%, 25%, 15%) 적용:**")
        
        korea_score = 0*0.6 + 85*0.25 + 95*0.15
        germany_score = 54.4*0.6 + 100*0.25 + 90*0.15
        china_score = 100*0.6 + 45*0.25 + 30*0.15
        
        # 계산 과정을 단계별로 표시
        calculation_data = {
            '국가': ['한국', '독일', '중국'],
            '계산식': [
                '0×0.6 + 85×0.25 + 95×0.15',
                '54.4×0.6 + 100×0.25 + 90×0.15',
                '100×0.6 + 45×0.25 + 30×0.15'
            ],
...
def main():
    ...
    clustered_df, kmeans_model, scaler = perform_clustering(analyzed_df)
     ...
     # 탭 선언: 반드시 7개 변수로 받아야 함!
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        "📊 종합 대시보드", "🎯 전략별 분석", "🔍 군집 분석", 
        "📈 성장성 분석", "⚠️ 리스크 분석", "🎮 시뮬레이션", "ℹ️ 모델 설명"
    ])
